...
function UserListRow(props) {
  const { _id, username, fullName, email, phone, password, bookedEvents } =
    props.obj; //Object destruction

  const handleClick = () => {
    Axios.all([
      Axios.delete(
        "https://event-management-system-proj.onrender.com/eventRoute/delete-user/" +
          _id
      )
        .then((res) => {
          if (res.status === 200) {
