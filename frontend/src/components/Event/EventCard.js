...
function EventCard(props) {
  ...
  const Book = () => {
    Axios.get("http://localhost:4000/eventRoute/check-user/" + user)
      .then((res) => {
        if (res.status === 200) {
          if (res.data != null) {
            const check = res.data.bookedEvents.some(
              (e) => e._id === props.obj._id
            );
            if (check) {
  ...
  const viewRegisteredUsers = () => {
    setDescription(
      ...
      >
        ...
        <tbody>{registeredUserItems()}</tbody>
      </table>
    );
    setUpdateButton(
      <button
  ...
  return (
    <Card className="eventCard">
      <Card.Body>
        <Card.Title style={{ fontSize: "2vw", fontWeight: "bolder" }}>
          {name}
        </Card.Title>
        <Card.Subtitle
          style={{ fontSize: "1.3vw", fontWeight: "bold", fontStyle: "italic" }}
        >
          {club}
        </Card.Subtitle>
        {desc}
